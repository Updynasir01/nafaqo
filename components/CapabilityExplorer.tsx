  ...
  return (
    <div className="grid grid-cols-1 items-start gap-10 md:grid-cols-2">
      <div className="border-t border-divider">
        {capabilities.map((capability, index) => {
          ...
          return (
            ...
            >
              <span className="flex items-center gap-2 text-[14px] font-extrabold text-gold-700">
                <Icon name={capability.icon as IconName} size={18} className="text-green-600" />
                {capability.num}
              </span>
              <span className="text-[19px] font-extrabold leading-[1.25]">{capability.title}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="flex min-h-[340px] flex-col rounded-lg bg-green-800 p-9 text-sand-100 md:sticky md:top-[110px]">
        ...
        <h2 className="mb-4 mt-4 text-[clamp(24px,2.8vw,34px)] font-extrabold leading-[1.12]">{item.title}</h2>
