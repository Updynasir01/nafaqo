  ...
  return (
    <section className="border-b border-divider">
      <Shell className="py-14 md:py-20">
        ...
        <h1 className="mt-6 text-[clamp(32px,4.6vw,56px)] font-semibold leading-[1.0]">{title}</h1>
        {standfirst ? (
...
export function Quote({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <blockquote
      className={
        "border-l-[3px] border-gold-400 pl-4 text-[clamp(18px,2vw,24px)] font-semibold leading-[1.24] " + className
      }
    >
      {children}
    </blockquote>
